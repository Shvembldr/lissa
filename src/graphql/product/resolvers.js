...
export default () => ({
  Query: {
    products: isAuthenticatedResolver.createResolver(
      async (obj, { match, filters, dates, limit, offset }) => {
    ...
    productsReport: isAdminResolver.createResolver(
      async (obj, { dateRange }) => {
        ...
        const productsHash = products.reduce((acc, product) => {
          ...
          if (!acc.vendorCode) {
            acc[vendorCode] = {
              count: 0,
              price: Operations.reduce(
                (sum, op) => sum + op.dataValues.price,
                0
              )
            };
          }
  ...
  Mutation: {
    createProduct: isAuthenticatedResolver.createResolver(
      async (obj, { input }) => {
    ...
    updateProduct: isAuthenticatedResolver.createResolver(
      async (obj, { id, input }) => {
        const card = await models.Card.findOne({
          where: {
            vendorCode: input.vendorCode
    ...
    removeProduct: isAuthenticatedResolver.createResolver(
      async (obj, { id }) => {
        const product = await models.Product.findById(id);
    ...
    removeProducts: isAuthenticatedResolver.createResolver(
      async (obj, { ids }) => {
        const products = ids.map(id => models.Product.findById(id));
        const resolvedProducts = await Promise.all(products);
        const productsToDestroy = resolvedProducts.map(product =>
          product.destroy()
        );
