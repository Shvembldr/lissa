...
export default () => ({
  Query: {
    ...
    cards: isAuthenticatedResolver.createResolver(
      async (obj, { match, filters, limit, offset }) => {
  ...
  Mutation: {
    createCard: isAuthenticatedResolver.createResolver(
      async (obj, { input, operationCount }) => {
        const card = await models.Card.create(input);
        const operationsData = new Array(operationCount)
          .fill(null)
          .map((data, i) => ({
            code: i + 1,
            price: 0,
    ...
    updateCard: isAuthenticatedResolver.createResolver(
      async (obj, { id, input }) => {
    ...
    removeCards: isAuthenticatedResolver.createResolver(
      async (obj, { ids }) => {
