...
export default () => ({
  Query: {
    customers: isAuthenticatedResolver.createResolver(() =>
  ...
  Mutation: {
    createCustomer: isAdminResolver.createResolver((obj, { input }) =>
      models.Customer.create(input)
    ),

    updateCustomer: isAdminResolver.createResolver(
      async (obj, { id, input }) => {
    ...
    removeCustomers: isAdminResolver.createResolver(async (obj, { ids }) => {
      ...
      const customersToDestroy = resolvedCustomers.map(customer =>
        customer.destroy()
      );
